...
(function ($, window, document) {
    ...
    PeriodPicker.prototype.calcDate = function (date) {
        date.setFullYear(this.year);
        date.setMonth(this.month - 1); // set month after year, because month can be more then 12 when work mousewheel 
        date.setDate(1);
    };
    ...
    PeriodPicker.prototype.init = function () {
        ...
        if (that.options.clearButtonInButton) {
            that.button.find('.icon_clear').on('click', function (e) {
                that.clear();
                e.preventDefault();
                e.stopPropagation();
                return false;
        ...
        if (!that.options.noHideSourceInputs) {
            ...
            that
                .startinput
                .add(that.endinput)
                .on('chenge keydown', function () {
                    clearTimeout(timer);
                    timer = setTimeout(function () {
                        var value = that.getInputsValue(), date;
                        if ((value[0] !== undefined && value[0] !== that.startinput.val()) || (value[1] !== undefined && this.endinput.length && value[1] !== that.endinput.val())) {
                            date = new TimeWrapper();
                            that.addRange([date.parseStr(that.startinput.val(), that.options.formatDateTime), date.parseStr(that.endinput.val(), that.options.formatDateTime)]);
                        }
                    }, 300);
                });
        }
