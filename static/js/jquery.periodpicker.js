...
(function ($, window, document) {
    ...
    function PeriodPicker(startinput, opt) {
        ...
        if (that.startinput.attr('id')) {
            that.picker.attr('data-input_id', that.startinput.attr('id'));
            that.button.attr('data-input_id', that.startinput.attr('id'));
        }
        ...
        var applyOptions = function () {
            ...
            addRange = function (value) {
                ...
                if (options.yearsPeriod[0] !== null && that.year < options.yearsPeriod[0]) {
                    options.yearsPeriod[0] = that.year - 10;
                }

                if (options.yearsPeriod[1] !== null && that.year > options.yearsPeriod[1]) {
                    options.yearsPeriod[1] = that.year + 10;
                }
            ...
            recalcDraggerPosition = function () {
                ...
                if (options.defaultTimeout) {
                    that.timer2 = setTimeout(calc, options.defaultTimeout);
                } else {
                    calc();
                }
            },
            ...
            init = function () {
                ...
                if (options.mousewheel) {
                    that.picker.on('mousewheel', function (e) {
                        var delta = deltaY(e);
                        that.month += (options.reverseMouseWheel ? -1 : 1) * delta;
                        regenerate();
                        return false;
                    });
                    if (options.mousewheelYearsLine) {
                        that.picker.find('.period_picker_years_selector').on('mousewheel', function (e) {
                            var delta = deltaY(e);
                            that.year += (options.reverseMouseWheel ? -1 : 1) * delta;
                            that.month = 1;
                ...
                that.picker.on('touchstart mousedown', '.period_picker_show.period_picker_today', function () {
                ...
                that.picker.on('touchstart mousedown', '.period_picker_show.period_picker_ok', function () {
                ...
                if (options.clearButtonInButton) {
                    ...
                    that.button.find('.icon_clear').on('touchstart mousedown', function (e) {
                        clear();
                ...
                if (options.clearButton) {
                    that.picker.on('touchstart mousedown', '.period_picker_show.period_picker_clear', function () {
                        clear();
                    });
                }
                ...
                if (options.timepicker && $.fn.TimePicker !== undefined) {
                    that.timepicker.each(function () {
                        ...
                        if ($that.length && !$that.data('timepicker') && $.fn.TimePicker !== undefined) {
                            // init timepicker
                            var timepickerOptions = $.extend(true, {}, options.timepickerOptions);
                            if (index && options.defaultEndTime) {
                                timepickerOptions.defaultTime = options.defaultEndTime;
                            }
                            $that.TimePicker(timepickerOptions, $(this));
            ...
            getPosition = function (target) {
                var offset = !options.likeXDSoftDateTimePicker ? that.button.offset() : $(target || that.startinput).offset(),
                    top = 0,
                    left = 0;
        ...
        if (options.timepicker && $.fn.TimePicker !== undefined) {
            var getTime = function (str) {
                if (str) {
                    return date.parseStr(str, options.formatDateTime) || date.parseStr(str, options.formatDate)
                }

                return null;
            };
            addRangeTime(
                getTime(values[0]) || getTime(options.timepickerOptions.defaultTime),
                getTime(values[1]) || getTime(options.defaultEndTime) || getTime(options.timepickerOptions.defaultTime)
            );
        }
    ...
    PeriodPicker.prototype.setValue = function (opt2) {
        var date = new TimeWrapper(), times = [];
        if ($.isArray(opt2)) {
            times[0] = date.parseStr(opt2[0], !this.options.timepicker ? this.options.formatDate : this.options.formatDateTime) || date.parseStr(opt2[0], this.options.formatDate);
            if (opt2[1] !== undefined && opt2[1]) {
                times[1] = date.parseStr(opt2[1], !this.options.timepicker ? this.options.formatDate : this.options.formatDateTime) || date.parseStr(opt2[1], this.options.formatDate);
            }
        } else {
            times[0] = date.parseStr(opt2, !this.options.timepicker ? this.options.formatDate : this.options.formatDateTime) || date.parseStr(opt2, this.options.formatDate);
        }
        this.addRange(times);
        if (this.options.timepicker && $.fn.TimePicker !== undefined) {
            this.addRangeTime(times[0], times[1] || times[0]);
        }
    };
        ...
        this.each(function () {
            ...
            if (!periodpicker) {
                ...
                case 'change':
                    ...
                    // }
                    periodpicker.setValue([
                        periodpicker.startinput.val(),
                        periodpicker.endinput.val()
                    ]);
