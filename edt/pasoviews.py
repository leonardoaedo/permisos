...
def permisolst(request):
    ...
    if  usuarioObj.rol.nivel_acceso == 0:
        ...
        gerente = Funcion.objects.get(id=26)
        resolutor = Resolucion.objects.values_list("permiso").filter(resolutor=gerente)

        permiso = Permiso.objects.annotate(num_b=Count('resolucion')).filter(num_b__lte=2).exclude(id__in=gerente).exclude(id__in=anulados).exclude(id__in=rechazados).order_by("-fecha_creacion") 
